...
contract BenchmarkSolidity is Script {
    ...
    function run() public {
...

        {
            for (uint256 i = 0; i < runCount; i++) {
                ...
                // Solidity Deposit
                console.log("Solidity Deposit...");
                vm.startBroadcast();
                IBatchDeposit(0x9b8c989FF27e948F55B53Bb19B3cC1947852E394).batchDeposit{value: 32 ether * count}(
                    publicKeys, withdrawalCredentials, signatures, depositDataRoots
                );
                vm.stopBroadcast();
            }
        }
    ...
    function publicKeys(uint256 count) public pure returns (bytes[] memory) {
        ...
        for (uint256 i = 0; i < count; i++) {
            publicKeys[i] = genPubkey();
        }
        return publicKeys;
    }

    function withdrawalCredentials(uint256 count) public pure returns (bytes[] memory) {
        bytes[] memory withdrawalCredentials = new bytes[](count);
        for (uint256 i = 0; i < count; i++) {
            withdrawalCredentials[i] = genWC();
        }
